...
OSDefineMetaClassAndStructors(BrcmFM, IOEthernetController)
OSDefineMetaClassAndStructors(CTimeout, OSObject)

IOWorkLoop *_fWorkloop;
IOCommandGate *_fCommandGate;

bool BrcmFM::init(OSDictionary *properties)
{
    XYLog("BrcmFM: %s\n", __FUNCTION__);
...
IOService* BrcmFM::probe(IOService *provider, SInt32 *score)
{
    XYLog("BrcmFM: %s\n", __FUNCTION__);
    bool isMatch = false;
    ...
    if (bwfm_pci_match(device)) {
        isMatch = true;
        fHalService = new Bwfm;
    }
    if (isMatch) {
...
bool BrcmFM::configureInterface(IONetworkInterface *netif) {
    IONetworkData *nd;
...
IONetworkInterface *BrcmFM::createInterface()
{
    BrcmFMInterface *netif = new BrcmFMInterface;
    if (!netif) {
        return NULL;
    }
    if (!netif->init(this)) {
...
struct _ifnet *BrcmFM::getIfp()
{
    XYLog("BrcmFM: %s\n", __FUNCTION__);
    return &fHalService->get80211Controller()->ic_ac.ac_if;
}

IOEthernetInterface *BrcmFM::getNetworkInterface()
{
    return getIfp()->iface;
}

bool BrcmFM::createMediumTables(const IONetworkMedium **primary)
{
    XYLog("BrcmFM: %s\n", __FUNCTION__);
...
void BrcmFM::joinSSID(const char *ssid_name, const char *ssid_pwd)
{
    XYLog("BrcmFM: %s\n", __FUNCTION__);
...
void BrcmFM::associateSSID(const char *ssid, const char *pwd)
{
    struct ieee80211com *ic = fHalService->get80211Controller();
    if (strlen(pwd) == 0) {
...
bool BrcmFM::start(IOService *provider)
{
    XYLog("BrcmFM: %s\n", __FUNCTION__);
    ...
    _fCommandGate = IOCommandGate::commandGate(this, (IOCommandGate::Action)BrcmFM::tsleepHandler);
    if (_fCommandGate == 0) {
    ...
    watchdogTimer = IOTimerEventSource::timerEventSource(this, OSMemberFunctionCast(IOTimerEventSource::Action, this, &BrcmFM::watchdogAction));
    if (!watchdogTimer) {
...
void BrcmFM::watchdogAction(IOTimerEventSource *timer)
{
    XYLog("BrcmFM: %s\n", __FUNCTION__);
...
const OSString * BrcmFM::newVendorString() const
{
    return OSString::withCString("Apple");
}

const OSString * BrcmFM::newModelString() const
{
    return OSString::withCString("Intel Wireless Card");
}

bool BrcmFM::initPCIPowerManagment(IOPCIDevice *provider)
{
    XYLog("BrcmFM: %s\n", __FUNCTION__);
    UInt16 reg16;
...
IOWorkLoop *BrcmFM::getWorkLoop() const
{
    return _fWorkloop;
}

IOReturn BrcmFM::selectMedium(const IONetworkMedium *medium) {
    setSelectedMedium(medium);
    return kIOReturnSuccess;
}

void BrcmFM::stop(IOService *provider)
{
    XYLog("BrcmFM: %s\n", __FUNCTION__);
...
IOReturn BrcmFM::enable(IONetworkInterface *netif)
{
    XYLog("%s\n", __FUNCTION__);
    super::enable(netif);
...
IOReturn BrcmFM::disable(IONetworkInterface *netif)
{
    XYLog("%s\n", __FUNCTION__);
    super::disable(netif);
    watchdogTimer->cancelTimeout();
...
IOReturn BrcmFM::getHardwareAddress(IOEthernetAddress *addrP)
{
    XYLog("BrcmFM: %s\n", __FUNCTION__);
...
#ifdef __PRIVATE_SPI__
IOReturn BrcmFM::outputStart(IONetworkInterface *interface, IOOptionBits options)
{
...
UInt32 BrcmFM::outputPacket(mbuf_t m, void *param)
{
//    XYLog("%s\n", __FUNCTION__);
...
UInt32 BrcmFM::getFeatures() const
{
    return fHalService->getDriverInfo()->supportedFeatures();
}

IOReturn BrcmFM::setPromiscuousMode(IOEnetPromiscuousMode mode)
{
    return kIOReturnSuccess;
}

IOReturn BrcmFM::setMulticastMode(IOEnetMulticastMode mode)
{
    return kIOReturnSuccess;
}

IOReturn BrcmFM::setMulticastList(IOEthernetAddress* addr, UInt32 len)
{
    return fHalService->getDriverController()->setMulticastList(addr, len);
}

IOReturn BrcmFM::getPacketFilters(const OSSymbol *group, UInt32 *filters) const
{
...
{
    XYLog("BrcmFM: %s\n", __FUNCTION__);
    BrcmFM* dev = OSDynamicCast(BrcmFM, owner);
    if (dev == 0)
        return kIOReturnError;
