...
void     bwfm_start(struct _ifnet *);
void     bwfm_init(struct _ifnet *);
void     bwfm_stop(struct _ifnet *);
void     bwfm_iff(struct bwfm_softc *);
void     bwfm_watchdog(struct _ifnet *);
void     bwfm_update_node(void *, struct ieee80211_node *);
void     bwfm_update_nodes(struct bwfm_softc *);
int     bwfm_ioctl(struct _ifnet *, u_long, caddr_t);
int     bwfm_media_change(struct _ifnet *);
...
int     bwfm_proto_bcdc_set_dcmd(struct bwfm_softc *, int,
         int, char *, size_t);
void     bwfm_proto_bcdc_rx(struct bwfm_softc *, mbuf_t,
         struct mbuf_list *);
...
#endif
void     bwfm_rx_event(struct bwfm_softc *, mbuf_t);
...
bwfm_attach(struct bwfm_softc *sc)
{
    ...
    //sc->sc_taskq = taskq_create(DEVNAME(sc), 1, IPL_NET, 0);
    sc->sc_taskq = taskq_create(DEVNAME(sc), 1, IPL_NET, 0);
    task_set(&sc->sc_task, bwfm_task, sc, "bwfm_task");
    ml_init(&sc->sc_evml);
...
//	    return false;
//    sc->sc_initialized = 1;
//    XYLog("bwfm_attachhook: sc->sc_initialized = %d\n", sc->sc_initialized);
//    return true;
//}
...
bwfm_preinit(struct bwfm_softc *sc)
{
    ...
    struct _ifnet *ifp = &ic->ic_if;
    int i, j, nbands;
    uint32_t nmode, vhtmode;
    uint32_t bandlist[3], tmp;
    
    XYLog("bwfm_preinit: bwfm_softc->sc_initialized = %d\n", sc->sc_initialized);
    if (sc->sc_initialized)
	    return 0;

    if (bwfm_fwvar_cmd_get_int(sc, BWFM_C_GET_VERSION, &tmp)) {
	    XYLog("%s: could not read io type\n", DEVNAME(sc));
	    return 1;
    } else {
	    sc->sc_io_type = tmp;
    }
    if (bwfm_fwvar_var_get_data(sc, "cur_etheraddr", ic->ic_myaddr,
        sizeof(ic->ic_myaddr))) {
	    XYLog("%s: could not read mac address\n", DEVNAME(sc));
	    return 1;
    }
    ...
    if (bwfm_fwvar_cmd_get_data(sc, BWFM_C_GET_BANDLIST, bandlist,
        sizeof(bandlist))) {
	    XYLog("%s: couldn't get supported band list\n", DEVNAME(sc));
	    return 1;
    }
    ...
    if (if_setlladdr(ifp, ic->ic_myaddr))
	    XYLog("%s: could not set MAC address\n", DEVNAME(sc));
...
bwfm_init(struct _ifnet *ifp)
{
    ...
    XYLog("bwfm_init: bwfm_softc->sc_initialized = %d\n", sc->sc_initialized);
    if (!sc->sc_initialized) {
	    if (sc->sc_bus_ops->bs_preinit != NULL &&
	        sc->sc_bus_ops->bs_preinit(sc)) {
    	    XYLog("%s: could not init bus\n", DEVNAME(sc));
    	    return;
	    }
	    if (bwfm_preinit(sc)) {
    	    XYLog("%s: could not init\n", DEVNAME(sc));
    	    return;
	    }
	    sc->sc_initialized = 1;
    }
    ...
    if (bwfm_fwvar_var_set_data(sc, "join_pref", join_pref,
        sizeof(join_pref))) {
	    XYLog("%s: could not set join pref\n", DEVNAME(sc));
	    return;
    }
...
void
bwfm_watchdog(struct _ifnet *ifp)
{
    XYLog("bwfm: %s\n", __FUNCTION__);
    struct bwfm_softc *sc = (struct bwfm_softc*)ifp->if_softc;
...
int
bwfm_ioctl(struct _ifnet *ifp, u_long cmd, caddr_t data)
{
    XYLog("bwfm: %s\n", __FUNCTION__);
    struct bwfm_softc *sc = (struct bwfm_softc*)ifp->if_softc;
    struct ieee80211com *ic = &sc->sc_ic;
...
int
bwfm_media_change(struct _ifnet *ifp)
{
    XYLog("bwfm: %s\n", __FUNCTION__);
    int error;
...
bwfm_chip_attach(struct bwfm_softc *sc)
{
    ...
    if (sc->sc_buscore_ops->bc_prepare(sc) != 0) {
	    XYLog("%s: failed buscore prepare\n", DEVNAME(sc));
	    return 1;
    }
    ...
    case BWFM_CHIP_CHIPID_TYPE_SOCI_SB:
	    XYLog("%s: SoC interconnect SB not implemented\n",
	        DEVNAME(sc));
	    return 1;
    case BWFM_CHIP_CHIPID_TYPE_SOCI_AI:
	    ...
	    break;
    default:
	    XYLog("%s: SoC interconnect %d unknown\n",
    ...
    if (!cpu_found) {
	    XYLog("%s: CPU core not detected\n", DEVNAME(sc));
	    return 1;
    }
    if (need_socram && !has_socram) {
	    XYLog("%s: RAM core not provided\n", DEVNAME(sc));
	    return 1;
    }
...
bwfm_chip_dmp_erom_scan(struct bwfm_softc *sc)
{
    ...
    while (type != BWFM_DMP_DESC_EOT) {
	    ...
	    if (type != BWFM_DMP_DESC_COMPONENT) {
    	    XYLog("%s: not component descriptor\n", DEVNAME(sc));
    	    return;
	    }
...
bwfm_proto_bcdc_txctl(struct bwfm_softc *sc, int reqid, char *buf, size_t *len)
{
    ...
    Bwfm *that = (Bwfm *)((struct bwfm_pci_softc *)sc)->bwfm;
    int timeout = 0;
    ...
    if (that->tsleep_nsec(ctl, PWAIT, "bwfm", SEC_TO_NSEC(1)))
	    timeout = 1;
...
void
bwfm_proto_bcdc_rx(struct bwfm_softc *sc, mbuf_t m, struct mbuf_list *ml)
{
    XYLog("bwfm: %s\n", __FUNCTION__);
    ...
    if (mbuf_len(m) < sizeof(*hdr) + (hdr->data_offset << 2)) {
	    mbuf_freem(m);
	    return;
    }
    mbuf_adj(m, sizeof(*hdr) + (hdr->data_offset << 2));
...
{
    XYLog("bwfm: %s\n", __FUNCTION__);
    return sc->sc_proto_ops->proto_query_dcmd(sc, 0, cmd, (char *)data, &len);
}
...
{
    XYLog("bwfm: %s\n", __FUNCTION__);
    return sc->sc_proto_ops->proto_set_dcmd(sc, 0, cmd, (char *)data, len);
}
...
bwfm_newbuf(void)
{
    ...
    mbuf_gethdr(MBUF_DONTWAIT, MBUF_TYPE_DATA, &m);
    if (m == NULL)
	    return (NULL);

    //MCLGET(m, M_DONTWAIT);
    mbuf_mclget(MBUF_DONTWAIT, MBUF_TYPE_DATA, &m);
    ...
    mbuf_pkthdr_setlen(m, MCLBYTES);
    mbuf_setlen(m, MCLBYTES);
    return (m);
}

void
bwfm_rx(struct bwfm_softc *sc, mbuf_t m, struct mbuf_list *ml)
{
    XYLog("bwfm: %s\n", __FUNCTION__);
    struct ieee80211com *ic = &sc->sc_ic;
    struct _ifnet *ifp = &ic->ic_if;
...
void
bwfm_rx_event(struct bwfm_softc *sc, mbuf_t m)
{
    XYLog("bwfm: %s\n", __FUNCTION__);
    int s;
...
void
bwfm_rx_event_cb(struct bwfm_softc *sc, mbuf_t m)
{
    XYLog("bwfm: %s\n", __FUNCTION__);
    struct ieee80211com *ic = &sc->sc_ic;
    struct _ifnet *ifp = &ic->ic_if;
    ...
    void (*cb)(struct bwfm_softc *, void *), void *arg, int len)
{
    ...
    s = splnet();
    KASSERT(ring->queued < BWFM_HOST_CMD_RING_COUNT, "ring->queued < BWFM_HOST_CMD_RING_COUNT");
    ...
    cmd->cb = cb;
    KASSERT(len <= sizeof(cmd->data), "len <= sizeof(cmd->data)");
...
bwfm_set_key_cb(struct bwfm_softc *sc, void *arg)
{
    ...
    case IEEE80211_CIPHER_CCMP:
	    ...
	    break;
    default:
	    XYLog("%s: cipher %x not supported\n", DEVNAME(sc),
	        k->k_cipher);
	    ieee80211_new_state(ic, IEEE80211_S_SCAN, -1);
	    return;
    }
...
bwfm_process_clm_blob(struct bwfm_softc *sc)
{
    ...
    while (remain) {
	    ...
	    if (bwfm_fwvar_var_set_data(sc, "clmload", data,
	        sizeof(*data) + len)) {
    	    XYLog("%s: could not load CLM blob\n", DEVNAME(sc));
    	    goto out;
	    }
    ...
    u_char **ucode, size_t *size, u_char **nvram, size_t *nvsize, size_t *nvlen)
{
    ...
    *size = *nvsize = *nvlen = 0;
    ...
    fwData = getFWDescByName(name);
    if (fwData == NULL) {
        err = 1;
        XYLog("%s resource load fail.\n", name);
        return err;
    }
    *size = fwData->getLength() * 4;
    *ucode = (u_char *)malloc(*size, 1, 1);
    uncompressFirmware(*ucode, (uint *)size, (u_char *)fwData->getBytesNoCopy(), fwData->getLength());
...
//    	    XYLog("%s: failed loadfirmware of file %s\n",
//    	        DEVNAME(sc), name);
//    	    return 1;
//	    }
//    }
...
//	    	    XYLog("%s: failed to process file %s\n",
//	    	        DEVNAME(sc), name);
//	    	    free(*ucode);
//	    	    free(*nvram);
//	    	    return 1;
