...
#define BWFM_PCI_DMA_MAP(_bdm)    (NULL)
#define BWFM_PCI_DMA_LEN(_bdm)    ((_bdm)->size)
#define BWFM_PCI_DMA_DVA(_bdm)    ((uint64_t)(_bdm)->paddr)
#define BWFM_PCI_DMA_KVA(_bdm)    ((char *)(_bdm)->vaddr)
...
int		 bwfm_pci_intr(OSObject *object, IOInterruptEventSource* sender, int count);
...
int		 bwfm_pci_pktid_new(struct bwfm_pci_softc *,
		    struct bwfm_pci_pkts *, mbuf_t,
		    uint32_t *, bus_addr_t *);
mbuf_t	 bwfm_pci_pktid_free(struct bwfm_pci_softc *,
...
int		 bwfm_pci_txdata(struct bwfm_softc *, mbuf_t);
...
int
bwfm_pci_match(IOPCIDevice *device)
{
    int devId = device->configRead16(kIOPCIConfigDeviceID);
    return pci_matchbyid(PCI_VENDOR_BROADCOM, devId, bwfm_pci_devices,
                         nitems(bwfm_pci_devices));
}

bool
bwfm_pci_attach(struct bwfm_pci_softc *sc, struct pci_attach_args *pa)
{
    XYLog("if_bwfm_pci: %s\n", __FUNCTION__);
    //struct bwfm_pci_softc *sc = (struct bwfm_pci_softc *)self;
    //struct bwfm_softc *sc_sc = &sc->sc_sc;
    ...
    if (pci_intr_map_msi(pa, &sc->ih) != 0 && pci_intr_map(pa, &sc->ih) != 0) {
	    XYLog(": couldn't map interrupt\n");
	    goto bar0;
    }
...
//    if (sc->sc_ih == NULL) {
//	    XYLog(": couldn't establish interrupt");
//	    if (intrstr != NULL)
//    	    XYLog(" at %s", intrstr);
//	    XYLog("\n");
//	    goto bar1;
...
//    XYLog(": %s\n", intrstr);
    ...
    XYLog("msiIntrIndex=%d\n", msiIntrIndex);
    //sc->sc_ih = IOInterruptEventSource::interruptEventSource(sc->sc_sc.sc_ic.ic_ac.ac_if.controller,
    sc->sc_ih = IOInterruptEventSource::interruptEventSource(sc->bwfm,
                                                             (IOInterruptEventSource::Action)&bwfm_pci_intr,
                                                             pa->pa_tag,
                                                             msiIntrIndex);
    ...
    //return bwfm_attachhook(&sc->sc_sc);

    
    if (bwfm_pci_preinit((struct bwfm_softc *)sc))
        return false;
    if (bwfm_preinit((struct bwfm_softc *)sc))
        return false;
    sc->sc_initialized = 1;
    XYLog("bwfm_attachhook: bwfm_pci_softc->sc_initialized = %d\n", sc->sc_initialized);
    return true;
...
bwfm_pci_preinit(struct bwfm_softc *bwfm)
{
    ...
    XYLog("bwfm_pci_preinit: bwfm_pci_softc->sc_initialized = %d\n", sc->sc_initialized);
    if (sc->sc_initialized)
	    return 0;

    sc->sc_sc.sc_buscore_ops = &bwfm_pci_buscore_ops;
    if (bwfm_chip_attach(&sc->sc_sc) != 0) {
	    XYLog("%s: cannot attach chip\n", DEVNAME(sc));
	    return 1;
    }

#if defined(__HAVE_FDT)
    if (bwfm_pci_read_otp(sc)) {
	    XYLog("%s: cannot read OTP\n", DEVNAME(sc));
	    return 1;
    }
#endif
    ...
    case BRCM_CC_4378_CHIP_ID:
	    ...
	    break;
    default:
	    XYLog("%s: unknown firmware for chip %s\n",
    ...
    if (bwfm_pci_load_microcode(sc, ucode, size, nvram, nvlen) != 0) {
	    XYLog("%s: could not load microcode\n",
	        DEVNAME(sc));
	    free(ucode);
	    free(nvram);
	    return 1;
    ...
    if (sc->sc_shared_version > BWFM_SHARED_INFO_MAX_VERSION ||
	    ...
	    XYLog("%s: PCIe version %d unsupported\n",
    ...
    bus_space_read_region_1(sc->sc_tcm_iot, sc->sc_tcm_ioh,
        sc->sc_ring_info_addr, (u_int8_t *)&ringinfo, sizeof(ringinfo));
    ...
    if (sc->sc_dma_idx_sz == 0) {
	    ...
	    if (sc->sc_dma_idx_buf == NULL) {
    	    /* XXX: Fallback to TCM? */
    	    XYLog("%s: cannot allocate idx buf\n",
    	        DEVNAME(sc));
    	    return 1;
	    }
    ...
    sc->sc_flowrings = (struct bwfm_pci_msgring *)malloc(sc->sc_max_flowrings *
    ...
    sc->sc_rx_pkts.pkts = (struct bwfm_pci_buf *)malloc(BWFM_NUM_RX_PKTIDS *
    ...
    sc->sc_tx_pkts.pkts = (struct bwfm_pci_buf *)malloc(BWFM_NUM_TX_PKTIDS
        * sizeof(struct bwfm_pci_buf), M_DEVBUF, M_WAITOK | M_ZERO);
    ...
    sc->sc_ioctl_pkts.pkts = (struct bwfm_pci_buf*)malloc(BWFM_NUM_IOCTL_PKTIDS
        * sizeof(struct bwfm_pci_buf), M_DEVBUF, M_WAITOK | M_ZERO);
...
#endif
    sc->sc_initialized = 1;
    XYLog("bwfm_pci_preinit return: bwfm_pci_softc->sc_initialized = %d\n", sc->sc_initialized);
    return 0;
...
{
    XYLog("if_bwfm_pci: %s\n", __FUNCTION__);
    struct bwfm_softc *bwfm = (struct bwfm_softc *)sc;
...
next:
	    ...
	    XYLog("%s: firmware C-%s%s%s/P-%s_M-%s_V-%s__m-%s\n",
	        DEVNAME(sc), chip,
...
bool allocDmaMemory2(struct bwfm_pci_dmamem *dma, bus_size_t size, int alignment)
{
    IOBufferMemoryDescriptor *bmd;
...
{
    XYLog("if_bwfm_pci: bwfm_pci_dmamem_alloc\n");
    struct bwfm_pci_dmamem *bdm;

    bdm = (struct bwfm_pci_dmamem *)malloc(sizeof(*bdm), M_DEVBUF, M_WAITOK | M_ZERO);
    bdm->size = size;

    if (!allocDmaMemory2(bdm, size, (int)align)) {
        XYLog("allocDmaMemory error.\n");
        return (NULL);
    }
    return (bdm);
}
...
void
bwfm_pci_dmamem_free(struct bwfm_pci_softc *sc, struct bwfm_pci_dmamem *dma)
{
    XYLog("if_bwfm_pci: %s\n", __FUNCTION__);
...
//    mbuf_t m, uint32_t *pktid, bus_addr_t *paddr)
//{
//    XYLog("if_bwfm_pci: %s\n", __FUNCTION__);
//    int i, idx;
//
//    if (m == NULL) {
//        XYLog("could not allocate pktid mbuf\n");
//        return ENOMEM;
//    }
//    idx = pkts->last + 1;
//    for (i = 0; i < pkts->npkt; i++) {
//	    if (idx == pkts->npkt)
//    	    idx = 0;
//        struct bwfm_pci_buf *pci_buf = &pkts->pkts[idx];
//        if (pci_buf->bb_m == NULL) {
//            if (bus_dmamap_load_mbuf(pkts->pkts[idx].bb_map, m) != 0) {
...
//    	        0, pkts->pkts[idx].bb_map->dm_segs->length,
//    	        BUS_DMASYNC_PREREAD | BUS_DMASYNC_PREWRITE);
//            pci_buf->bb_map->cursor->getPhysicalSegments(m, &pci_buf->bb_map->dm_segs[0], 1);
//            if (pci_buf->bb_map->dm_nsegs == 0) {
//                mbuf_freem(m);
//                return ENOMEM;
//            }
//    	    pkts->last = idx;
//    	    pci_buf->bb_m = m;
//    	    *pktid = idx;
//            *paddr = pci_buf->bb_map->dm_segs[0].location;
//    	    return 0;
//	    }
//	    idx++;
//    }
...
//                BUS_DMASYNC_PREREAD | BUS_DMASYNC_PREWRITE);
            ...
            *pktid = idx;
            *paddr = pkts->pkts[idx].bb_map->dm_segs[0].location;
            return 0;
        }
        idx++;
    }
    ...
    uint32_t pktid)
{
    ...
    bus_dmamap_sync(sc->sc_dmat, pkts->pkts[pktid].bb_map, 0,
        pkts->pkts[pktid].bb_map->dm_segs->length,
    ...
    uint32_t msgtype)
{
    ...
    for (slots = if_rxr_get(rxring, 8); slots > 0; slots--) {
	    if (bwfm_pci_pktid_avail(sc, &sc->sc_rx_pkts))
    	    break;
	    req = (struct msgbuf_rx_ioctl_resp_or_event *)bwfm_pci_ring_write_reserve(sc, &sc->sc_ctrl_submit);
	    if (req == NULL)
    	    break;
	    //m = MCLGETL(NULL, M_DONTWAIT, MSGBUF_MAX_CTL_PKT_SIZE);
        mbuf_allocpacket(MBUF_WAITOK, MSGBUF_MAX_CTL_PKT_SIZE, &max_chunks, &m);
	    if (m == NULL) {
    	    bwfm_pci_ring_write_cancel(sc, &sc->sc_ctrl_submit, 1);
    	    break;
	    }
        //m.m_len = m->m_pkthdr.len = MSGBUF_MAX_CTL_PKT_SIZE;
        mbuf_pkthdr_setlen(m, MSGBUF_MAX_CTL_PKT_SIZE);
        mbuf_setlen(m, MSGBUF_MAX_CTL_PKT_SIZE);
...
bwfm_pci_fill_rx_buf_ring(struct bwfm_pci_softc *sc)
{
    ...
    for (slots = if_rxr_get(&sc->sc_rxbuf_ring, sc->sc_max_rxbufpost);
	    ...
	    if (bwfm_pci_pktid_avail(sc, &sc->sc_rx_pkts))
    	    break;
	    req = (struct msgbuf_rx_bufpost *)bwfm_pci_ring_write_reserve(sc, &sc->sc_rxpost_submit);
	    if (req == NULL)
    	    break;
	    //m = MCLGETL(NULL, M_DONTWAIT, MSGBUF_MAX_PKT_SIZE);
        mbuf_allocpacket(MBUF_DONTWAIT, MSGBUF_MAX_PKT_SIZE, &max_chunks, &m);
	    if (m == NULL) {
    	    bwfm_pci_ring_write_cancel(sc, &sc->sc_rxpost_submit, 1);
    	    break;
	    }
	    //m->m_len = m->m_pkthdr.len = MSGBUF_MAX_PKT_SIZE;
        mbuf_pkthdr_setlen(m, MSGBUF_MAX_PKT_SIZE);
        mbuf_setlen(m, MSGBUF_MAX_PKT_SIZE);
	    if (bwfm_pci_pktid_new(sc, &sc->sc_rx_pkts, m, &pktid, &paddr)) {
    	    bwfm_pci_ring_write_cancel(sc, &sc->sc_rxpost_submit, 1);
            XYLog("mbuf_freem(m)\n");
    	    m_freem(m);
    ...
    struct bwfm_pci_msgring *ring)
{
    ...
    if (sc->sc_dma_idx_sz == 0) {
	    ...
	    *(uint16_t *)((uint32_t *)(sc->sc_dma_idx_buf)
	        + ring->w_idx_addr) = ring->w_ptr;
	    bus_dmamap_sync(sc->sc_dmat,
	        BWFM_PCI_DMA_MAP(sc->sc_dma_idx_buf), ring->w_idx_addr,
	        sizeof(uint16_t), BUS_DMASYNC_PREREAD | BUS_DMASYNC_PREWRITE);
...
again:
    ...
    processed = 0;
    while (avail) {
        XYLog("bwfm_pci_ring_rx avail=%d processed=%d.\n", avail, processed);
	    bwfm_pci_msg_rx(sc, (char *)buf + sc->sc_rx_dataoffset, ml);
        buf = (char *)buf + ring->itemsz;
	    processed++;
	    if (processed == 48) {
...
{
    XYLog("if_bwfm_pci: %s\n", __FUNCTION__);
    struct _ifnet *ifp = &sc->sc_sc.sc_ic.ic_if;
    struct msgbuf_gen_status *gen;
    ...
    {
    case 0x1:
        gen = (struct msgbuf_gen_status *)buf;
            
        XYLog("Firmware reported general error: %d\n", letoh16(gen->compl_hdr.status));
        break;
    case MSGBUF_TYPE_FLOW_RING_CREATE_CMPLT:
	    ...
	    if (fcr->compl_hdr.status) {
    	    XYLog("%s: failed to open flowring %d\n",
    	        DEVNAME(sc), flowid);
    ...
    case MSGBUF_TYPE_FLOW_RING_DELETE_CMPLT:
	    ...
	    if (fdr->compl_hdr.status) {
    	    XYLog("%s: failed to delete flowring %d\n",
    	        DEVNAME(sc), flowid);
    	    break;
	    }
    ...
    case MSGBUF_TYPE_WL_EVENT:
	    ...
	    if (m == NULL)
    	    break;
	    mbuf_adj(m, sc->sc_rx_dataoffset);
	    //m->m_len = m->m_pkthdr.len = letoh16(event->event_data_len);
        mbuf_pkthdr_setlen(m, letoh16(event->event_data_len));
        mbuf_setlen(m, letoh16(event->event_data_len));
    ...
    case MSGBUF_TYPE_RX_CMPLT:
	    ...
	    if (letoh16(rx->data_offset))
    	    mbuf_adj(m, letoh16(rx->data_offset));
	    else if (sc->sc_rx_dataoffset)
    	    mbuf_adj(m, sc->sc_rx_dataoffset);
	    //m->m_len = m->m_pkthdr.len = letoh16(rx->data_len);
        mbuf_pkthdr_setlen(m, letoh16(rx->data_len));
        mbuf_setlen(m, letoh16(rx->data_len));
	    bwfm_rx(&sc->sc_sc, m, ml);
	    if_rxr_put(&sc->sc_rxbuf_ring, 1);
	    bwfm_pci_fill_rx_rings(sc);
	    break;
    default:
	    XYLog("%s: msgtype 0x%08x\n", __func__, msg->msgtype);
	    break;
    }
}
...
bwfm_pci_select_core(struct bwfm_pci_softc *sc, int id)
{
    ...
    if (core == NULL) {
	    XYLog("%s: could not find core to select", DEVNAME(sc));
	    return;
    }
...
int
bwfm_pci_flowring_lookup(struct bwfm_pci_softc *sc, mbuf_t m)
{
    XYLog("if_bwfm_pci: %s\n", __FUNCTION__);
...
void
bwfm_pci_flowring_create(struct bwfm_pci_softc *sc, mbuf_t m)
{
    XYLog("if_bwfm_pci: %s\n", __FUNCTION__);
    struct ieee80211com *ic = &sc->sc_sc.sc_ic;
...
bwfm_pci_flowring_create_cb(struct bwfm_softc *bwfm, void *arg)
{
    ...
    struct ieee80211com *ic = &sc->sc_sc.sc_ic;
#endif
    struct bwfm_cmd_flowring_create *cmd = (struct bwfm_cmd_flowring_create *)arg;
    struct msgbuf_tx_flowring_create_req *req;
    struct bwfm_pci_msgring *ring;
    uint8_t *da, *sa;
    int s;

    da = mtod(cmd->m, uint8_t *) + 0 * ETHER_ADDR_LEN;
    sa = mtod(cmd->m, uint8_t *) + 1 * ETHER_ADDR_LEN;

    ring = &sc->sc_flowrings[cmd->flowid];
    if (ring->status != RING_OPENING) {
	    XYLog("%s: flowring not opening\n", DEVNAME(sc));
	    return;
    }

    if (bwfm_pci_setup_flowring(sc, ring, 512, 48)) {
	    XYLog("%s: cannot setup flowring\n", DEVNAME(sc));
	    return;
    }

    s = splnet();
    req = (struct msgbuf_tx_flowring_create_req *)bwfm_pci_ring_write_reserve(sc, &sc->sc_ctrl_submit);
    if (req == NULL) {
	    XYLog("%s: cannot reserve for flowring\n", DEVNAME(sc));
	    splx(s);
	    return;
    }
...
bwfm_pci_flowring_delete(struct bwfm_pci_softc *sc, int flowid)
{
    ...
    if (ring->status != RING_OPEN) {
	    XYLog("%s: flowring not open\n", DEVNAME(sc));
	    return;
    }

    s = splnet();
    req = (struct msgbuf_tx_flowring_delete_req *)bwfm_pci_ring_write_reserve(sc, &sc->sc_ctrl_submit);
    if (req == NULL) {
	    XYLog("%s: cannot reserve for flowring\n", DEVNAME(sc));
	    splx(s);
	    return;
...
int
bwfm_pci_txdata(struct bwfm_softc *bwfm, mbuf_t m)
{
    XYLog("if_bwfm_pci: %s\n", __FUNCTION__);
    struct bwfm_pci_softc *sc = (struct bwfm_pci_softc *)bwfm;
    struct bwfm_pci_msgring *ring;
    ...
    if (ring->status == RING_OPENING ||
        ring->status == RING_CLOSING) {
	    XYLog("%s: tried to use a flow that was "
	        "transitioning in status %d\n",
	        DEVNAME(sc), ring->status);
	    return ENOBUFS;
    }

    tx = (struct msgbuf_tx_msghdr *)bwfm_pci_ring_write_reserve(sc, ring);
    if (tx == NULL)
	    return ENOBUFS;
    ...
    if (ret) {
	    if (ret == ENOBUFS) {
    	    XYLog("%s: no pktid available for TX\n",
    	        DEVNAME(sc));
    	    sc->sc_tx_pkts_full = 1;
	    }
    ...
    tx->data_len = htole16(mbuf_len(m) - ETHER_HDR_LEN);
...
int
bwfm_pci_intr(OSObject *object, IOInterruptEventSource* sender, int count)
{
    XYLog("if_bwfm_pci: bwfm_pci_intr.\n");
    Bwfm *that = (Bwfm*)object;
    struct bwfm_pci_softc *sc = &that->com;
    struct bwfm_softc *bwfm = (struct bwfm_softc *)sc;
    struct _ifnet *ifp = &sc->sc_sc.sc_ic.ic_if;
    struct mbuf_list ml = MBUF_LIST_INITIALIZER();
    uint32_t status, mask;
    
    XYLog("bwfm_pci_intr: bwfm_pci_softc->sc_initialized = %d\n", sc->sc_initialized);
    if (!sc->sc_initialized) {
        return 0;
    }
    ...
    int cmd, char *buf, size_t *len)
{
    ...
    Bwfm *that = (Bwfm *)sc->bwfm;
    //Bwfm *that = container_of(sc, Bwfm, com);
    mbuf_t m;
    uint32_t pktid;
    bus_addr_t paddr;
    size_t buflen;
    ...
    //mbuf_allocpacket(MBUF_DONTWAIT, buflen, &max_chunks, &m);
//    IOEthernetController* controller = bwfm->sc_ic.ic_ac.ac_if.controller;
//    controller->allocatePacket(buflen);
    m = bwfm->sc_ic.ic_ac.ac_if.controller->allocatePacket(buflen);
    if (m == NULL)
	    return 1;
    //m->m_len = m->m_pkthdr.len = buflen;
//    mbuf_pkthdr_setlen(m, buflen);
//    mbuf_setlen(m, buflen);
    ...
    s = splnet();
    req = (struct msgbuf_ioctl_req_hdr *)bwfm_pci_ring_write_reserve(sc, &sc->sc_ctrl_submit);
    if (req == NULL) {
	    splx(s);
	    m_freem(m);
	    return 1;
    ...
    ctl = (struct bwfm_pci_ioctl *)malloc(sizeof(*ctl), M_TEMP, M_WAITOK|M_ZERO);
    ...
    XYLog("req->input_buf_len = htole16(mbuf_len(m)) = %d\n", htole16(mbuf_len(m)));
    req->input_buf_len = htole16(mbuf_len(m));
    ...
    that->tsleep_nsec(ctl, PWAIT, "bwfm", SEC_TO_NSEC(1));
