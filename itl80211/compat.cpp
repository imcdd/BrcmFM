...
bus_space_read_region_1(bus_space_tag_t tag, bus_space_handle_t handle,
    bus_size_t offset, u_int8_t *values, size_t count)
{
    for (int i = 0; i < count; i++, offset++, values++) {
        *values = *((uint8_t*)(handle + offset));
    }
//    while ((int)--count >= 0)
